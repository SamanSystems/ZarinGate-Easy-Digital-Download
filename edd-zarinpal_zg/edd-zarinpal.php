...
add_filter( 'edd_payment_gateways', 'zp_zg_add_gateway' );

function zp_zg_cc_form () {
	do_action( 'zp_zg_cc_form_action' );
}
add_filter( 'edd_zarinpal_cc_form', 'zp_zg_cc_form' );

function zp_zg_process_payment ($purchase_data) {
	global $edd_options;
	...
	if ($payment) {
        ...
        $redirect_page = "https://www.zarinpal.com/pg/Transactions/StartPay/" . $result->Authority . "/ZarinGate"; 
...
add_action('edd_gateway_zarinpal', 'zp_zg_process_payment');

function zp_zg_verify() {
	global $edd_options;
...
add_action('init', 'zp_zg_verify');

function zp_zg_add_settings ($settings) {
...
add_filter('edd_settings_gateways', 'zp_zg_add_settings');
?>
